...
export function initGame(difficulty) {
    ...
    const fieldInit = getCleanField(settings.rows, settings.cols)
//    const field= assignMineCountsToCells(plantMines(fieldInit, mines,Object.keys(fieldInit.cells)[0]));
    if (settings) {
        return {
            type: INIT_GAME,
            payload: {
...
function cellClick(id) {
    return (dispatch, getState) => {
        const { game, field, game: { totalMines } } = getState().game;    
...
function plantMines(field, mineCount, firstCellId) {
    ...
    while (mineCount) {
        ...
        mineCells[id] = setCellAttribute(field.cells[id], 'isMine', true);
    }
    return {
        ...field,
        cells: {
    ...
    Object.keys(field.cells)
        .forEach(id => {
            ...
            if (!cell.isMine) {
                ...
                newField.cells[id] = setCellAttribute(cell, 'minesAround', mineCount);
            }
        });
...
function uncoverMines(field) {
    ...
    return cellKeys
        .filter(key => {
            ...
            return cell.isMine && !cell.exploded;
        })
...
function flagRemainingMines(field) {
    ...
    return cellKeys
        .filter(key => {
            const cell = field.cells[key];
            return cell.isMine && !cell.flagged;
        })
        .reduce((prevField, key) => {
            return {
                ...prevField,
...
function findMistakes(field) {
    ...
    return cellKeys
        .filter(key => {
            const cell = field.cells[key];
            return cell.flagged && !cell.isMine;
        })
        .reduce((prevField, key) => {
            return {
                ...prevField,
